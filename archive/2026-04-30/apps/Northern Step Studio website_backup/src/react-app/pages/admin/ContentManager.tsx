...
export default function ContentManager() {
  ...
  async function fetchPosts() {
    try {
      ...
      setErrorMessage(err instanceof Error ? err.message : "Failed to load blog posts");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      setStatusMessage(editingPost ? "Post updated successfully." : "Post created successfully.");
    } catch (err) {
      console.error("Failed to save post:", err);
      setErrorMessage(err instanceof Error ? err.message : "Failed to save post");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function togglePublish(post: BlogPost) {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => null);
        throw new Error(payload?.error || "Failed to update publish state");
      }

      await fetchPosts();
      setStatusMessage(post.is_published ? "Post unpublished." : "Post published.");
    } catch (err) {
      console.error("Failed to toggle publish:", err);
      setErrorMessage(err instanceof Error ? err.message : "Failed to update publish state");
    }
  }
    ...
    <div>
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground font-normal">Manage blog posts and announcements</p>
        </div>
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((index) => (
            <div key={index} className="card-dark-wise animate-pulse h-20" />
          ))}
        </div>
        ...
        <div className="space-y-3">
          {filteredPosts.map((post) => (
            <div
              key={post.id}
              className="card-dark-wise p-4 flex flex-col gap-4 sm:flex-row sm:items-center"
            >
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setFormData((prev) => ({ ...prev, cover_image: event.target.value || null }))
                  }
                  placeholder="https://..."
                  className="w-full px-4 py-2.5 bg-muted border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-accent/50"
                />
            ...
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(event) =>
                    setFormData((prev) => ({ ...prev, is_published: event.target.checked ? 1 : 0 }))
                  }
