...
interface IntegrationCheckDiagnostic {
  expected: string[];
  readPath: string;
...
interface IntegrationStatus {
  cloudflare: {
    ...
    diagnostics?: {
      d1Binding?: IntegrationCheckDiagnostic;
      d1Connectivity?: IntegrationCheckDiagnostic;
      r2Binding?: IntegrationCheckDiagnostic;
      assetsBinding?: IntegrationCheckDiagnostic;
      emailService?: IntegrationCheckDiagnostic;
    };
  };
  supabase: {
    ...
    diagnostics?: {
      url?: IntegrationCheckDiagnostic;
      anonKey?: IntegrationCheckDiagnostic;
      serviceRoleKey?: IntegrationCheckDiagnostic;
      projectRef?: IntegrationCheckDiagnostic;
      connection?: IntegrationCheckDiagnostic;
    };
  };
  auth: {
...
export default function FeatureToggles() {
  ...
  const renderDiagnosticLine = (label: string, diagnostic?: IntegrationCheckDiagnostic) => {
    if (!diagnostic) {
      return null;
    }

    const segments: string[] = [];
    if (diagnostic.detectedName) {
      segments.push(`Detected: ${diagnostic.detectedName}`);
    }
    if (diagnostic.reason) {
      segments.push(diagnostic.reason);
    }
    if (!diagnostic.configured || diagnostic.ready === false) {
      segments.push(`Expected: ${diagnostic.expected.join(" | ")}`);
      segments.push(`Read path: ${diagnostic.readPath}`);
    }
    ...
    return (
      <p className="text-xs text-muted-foreground">
        <span className="font-semibold text-foreground/80">{label}:</span> {segments.join(" | ")}
      </p>
    );
  };
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <div className="rounded-2xl border border-border bg-card-soft p-5">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div className="rounded-xl border border-border bg-background/40 px-3 py-3 text-xs text-muted-foreground">
                  <p className="mb-2 font-semibold text-foreground/80">Diagnostics</p>
                  {renderDiagnosticLine("D1 binding", integrationStatus.cloudflare.diagnostics?.d1Binding)}
                  {renderDiagnosticLine("D1 connectivity", integrationStatus.cloudflare.diagnostics?.d1Connectivity)}
                  {renderDiagnosticLine("R2 binding", integrationStatus.cloudflare.diagnostics?.r2Binding)}
                  {renderDiagnosticLine("Assets binding", integrationStatus.cloudflare.diagnostics?.assetsBinding)}
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-border bg-card-soft p-5">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="rounded-xl border border-border bg-background/40 px-3 py-3 text-xs text-muted-foreground">
                  <p className="mb-2 font-semibold text-foreground/80">Diagnostics</p>
                  {renderDiagnosticLine("Supabase URL", integrationStatus.supabase.diagnostics?.url)}
                  {renderDiagnosticLine("Supabase anon key", integrationStatus.supabase.diagnostics?.anonKey)}
                  {renderDiagnosticLine("Supabase service role key", integrationStatus.supabase.diagnostics?.serviceRoleKey)}
                  {renderDiagnosticLine("Supabase project ref", integrationStatus.supabase.diagnostics?.projectRef)}
                  {renderDiagnosticLine("Supabase connection", integrationStatus.supabase.diagnostics?.connection)}
                  <p className="mt-2">
                    Detected env aliases: URL={integrationStatus.supabase.resolvedEnv?.url ?? "none"}, Anon=
                    {integrationStatus.supabase.resolvedEnv?.anonKey ?? "none"}, ServiceRole=
                    {integrationStatus.supabase.resolvedEnv?.serviceRoleKey ?? "none"}, ProjectRef=
                    {integrationStatus.supabase.resolvedEnv?.projectRef ?? "none"}.
                  </p>
                </div>
              </div>
            </div>
