    ...
    useFocusEffect(
        useCallback(() => {
            if (currentGameId !== 'letter-recognition') {
                void startGame('letter-recognition');
            }
...
function LetterRecognitionInner({ onResetLevel, onRestartGame, letterCase, setLetterCase }: InnerProps) {
    ...
    React.useEffect(() => {
        ...
        const timer = setTimeout(() => {
            speak(t('letterRecognition.instruction', { letter: targetLetter.toLowerCase() }), { shouldLock: true });
        }, 300);
    ...
    const handleChoice = (chosen: string) => {
        ...
        if (chosen === targetLetter) {
            ...
            // Immediate success feedback: just say the letter clearly
