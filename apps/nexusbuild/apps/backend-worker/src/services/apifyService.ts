...
const MODEL_TOKEN_RE =
  /(?:\d{3,5}[a-z]{0,4}|x3d|xtx|xt|gre|ti|super|ddr[45]|nvme|am[45]|lga\d{4})/i;
...
const ACTOR_ID = 'Wa8AOJpMTOnV1V7uX';
...
export const isApifyConfigured = (token?: string) => Boolean(token && token.trim() !== '');

export const checkApifyConnection = async (token: string): Promise<boolean> => {
  if (!isApifyConfigured(token)) return false;
  try {
...
const setCachedProduct = (key: string, result: PCPartPickerProduct | null) => {
  productCache.set(key, { expiresAt: Date.now() + PRODUCT_CACHE_TTL_MS, result });
};

export async function searchPCParts(options: SearchOptions, token: string): Promise<PCPartPickerProduct[]> {
  if (!isApifyConfigured(token)) throw new Error('APIFY_API_TOKEN is not configured');

  const { searchPhrases = [], category, maxProducts = 20, maxReviews = 0, countryCode = 'us' } = options;
  const normalizedCategory = normalizeCategory(category);
  const normalizedSearchPhrases = Array.from(new Set(searchPhrases.flatMap(buildSearchPhrases)));

  const cacheKey = JSON.stringify({ searchPhrases: normalizedSearchPhrases, category: normalizedCategory, maxProducts, maxReviews, countryCode });
...
export async function getProductPrice(productName: string, category: string | undefined, token: string): Promise<PCPartPickerProduct | null> {
  ...
  try {
    ...
    const categoryCandidates = Array.from(new Set([normalizeCategory(category), undefined]));
    let bestProduct: PCPartPickerProduct | null = null;

    for (const categoryCandidate of categoryCandidates) {
      const results = await searchPCParts({ searchPhrases, category: categoryCandidate, maxProducts: 12, countryCode: 'us' }, token);
      ...
      if ((candidate.match?.score ?? 0) >= 70) break;
    }
...
export async function getBatchPrices(products: { name: string; category?: string }[], token: string): Promise<Map<string, PCPartPickerProduct | null>> {
  const results = new Map<string, PCPartPickerProduct | null>();
  const uniqueProducts = Array.from(new Set(products.map((p) => `${p.name}::${normalizeCategory(p.category) || ''}`))).map((key) => {
    const [name, normalizedCategory] = key.split('::');
    return { name, category: normalizedCategory || undefined, key };
  });

  const resolved = new Map<string, PCPartPickerProduct | null>();
  for (let i = 0; i < uniqueProducts.length; i += 4) {
    const batch = uniqueProducts.slice(i, i + 4);
    await Promise.all(
      batch.map(async (product) => {
        const result = await getProductPrice(product.name, product.category, token);
...
export async function getTrendingProducts(category: string, token: string, limit: number = 10): Promise<PCPartPickerProduct[]> {
  try {
    return await searchPCParts({ category, maxProducts: limit, countryCode: 'us' }, token);
  } catch (error) {
    return [];
  }
}
...
export function formatPrice(product: PCPartPickerProduct): string {
  if (product.price.lowestPrice == null) return 'Price not available';
