...
export default function GameHub() {
  ...
  const stats = useMemo(() => ({
    ...
    previewGames: games.filter(g => g.status === "PREVIEW").length,
  }), [games]);
    ...
    <div className="min-h-screen pt-24 px-4 sm:px-6 pb-12">
      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-8">
          ...
          <div className="card-dark-wise text-center py-4 sm:py-6">
            <div className="text-2xl sm:text-3xl font-black text-accent mb-1">{stats.previewGames}</div>
            <div className="text-xs sm:text-sm text-muted-foreground font-normal">{t("games.preview")}</div>
          </div>
        </div>
                  ...
                  {game.cta_url && (
                    <div className="mt-4 pt-4 border-t border-border">
                      <span className="btn-pill-primary w-full text-center text-xs sm:text-sm inline-flex items-center justify-center gap-2">
                        {game.status === "LIVE" ? t("games.play_now") : t("apps.join_preview")}
