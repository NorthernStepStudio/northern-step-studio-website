...
Write-Host "== NStep Android Build Center ==" -ForegroundColor Cyan
...
Write-Host "Android path: $AndroidPath"
...
try {
    ...
    if (!(Test-Path $PrivateKeystorePath)) {
      throw "Keystore not found: $PrivateKeystorePath. Run create-keystore.ps1 $AppName first."
    }

    if (!(Test-Path $AppAndroidPath)) {
      throw "Android app folder not found: $AppAndroidPath"
    }

    Write-Host "Copying keystore into android/app..." -ForegroundColor Yellow
...
"@
    ...
    try {
        ...
        $SearchRoot = Join-Path $AndroidPath "app\build\outputs\bundle"
        ...
        $FoundAab = Get-ChildItem -Path $SearchRoot -Recurse -Filter "*.aab" | 
        ...
        if ($FoundAab) {
          ...
          Write-Host ""
          Write-Host "AAB created and renamed successfully:" -ForegroundColor Green
          ...
          # 1. Search App Folder
          $DeepFound = Get-ChildItem -Path $AppPath -Recurse -Filter "*.aab" | 
            Where-Object { $_.Length -gt 1MB -and $_.LastWriteTime -ge $GracePeriod } |
            Sort-Object LastWriteTime -Descending | 
            Select-Object -First 1
          ...
          if (!$DeepFound) {
            ...
            if (Test-Path $RootB) {
              ...
              $DeepFound = Get-ChildItem -Path $RootB -Recurse -Filter "*.aab" | 
                Where-Object { $_.Length -gt 1MB -and $_.LastWriteTime -ge $GracePeriod } |
                Sort-Object LastWriteTime -Descending | 
                Select-Object -First 1
            }
          ...
          if ($DeepFound) {
            ...
            Write-Host "FAILED: Build finished but no valid AAB (>1MB) was found." -ForegroundColor Red
            exit 1
          }
        }
    } catch {
