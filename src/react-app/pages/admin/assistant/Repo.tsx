  ...
  const askAssistant = async () => {
    ...
    try {
      const res = await fetch("/api/admin/assistant/chat", {
        ...
        body: JSON.stringify({ mode: "repo", message: prompt }),
      });
    ...
    <div className="flex flex-col h-[600px]">
      <div className="flex-1 overflow-y-auto mb-4 p-4 border border-border rounded-lg bg-background">
        {response ? (
          ...
          <div className="text-muted-foreground text-sm flex items-center justify-center h-full gap-2">
            <FileCode className="w-5 h-5" />
            Ask about files, architecture, or generate fix prompts.
          </div>
        )}
        {loading && <div className="text-accent text-sm mt-2">Scanning repo...</div>}
      </div>
      <div className="flex gap-2">
        <input
          ...
          placeholder="Where is the NeuroMoves app?"
