...
type GenericToolRuntime = Partial<Stage3ToolRuntime>;

export async function executeAdapterToolStep(
  step: JobStepState,
  ...
  const tools = context.tools as GenericToolRuntime;
...
function parseInstruction(step: JobStepState): ToolExecutionInstruction | undefined {
  ...
  return {
    tool: tool as ToolExecutionInstruction["tool"],
    action,
    request,
...
async function dispatchInstruction(
  instruction: ToolExecutionInstruction,
  tools: GenericToolRuntime,
  context: WorkflowExecutionContext,
...
async function handleBrowserInstruction(
  instruction: ToolExecutionInstruction,
  tools: GenericToolRuntime,
): Promise<ToolExecutionResult | undefined> {
...
async function handleSmsInstruction(
  instruction: ToolExecutionInstruction,
  tools: GenericToolRuntime,
  context: WorkflowExecutionContext,
): Promise<ToolExecutionResult | undefined> {
  ...
  if (instruction.action === "send") {
    ...
    const result = await sms.send({
      ...
      status: readString(instruction.request, "status") as "queued" | "sent" | "delivered" | "failed" | "unknown" | undefined,
...
async function handleEmailInstruction(instruction: ToolExecutionInstruction, tools: GenericToolRuntime): Promise<ToolExecutionResult | undefined> {
...
async function handleDatabaseInstruction(instruction: ToolExecutionInstruction, tools: GenericToolRuntime): Promise<ToolExecutionResult | undefined> {
...
async function handleApiInstruction(instruction: ToolExecutionInstruction, tools: GenericToolRuntime): Promise<ToolExecutionResult | undefined> {
  const api = tools.api;
  if (!api) {
...
async function handleScrapingInstruction(instruction: ToolExecutionInstruction, tools: GenericToolRuntime): Promise<ToolExecutionResult | undefined> {
...
async function handleSchedulerInstruction(instruction: ToolExecutionInstruction, tools: GenericToolRuntime): Promise<ToolExecutionResult | undefined> {
...
async function handleRedisInstruction(instruction: ToolExecutionInstruction, tools: GenericToolRuntime): Promise<ToolExecutionResult | undefined> {
...
async function handleOcrInstruction(
  instruction: ToolExecutionInstruction,
  tools: GenericToolRuntime,
  context: WorkflowExecutionContext,
): Promise<ToolExecutionResult | undefined> {
...
function readAction(request: Record<string, unknown>): string | undefined {
  return readString(request, "action", "operation", "method");
}

function normalizeAction(tool: string, action: string | undefined): string | undefined {
  if (!action) {
    return undefined;
  }
...
function extractRequest(request: Record<string, unknown>): Record<string, unknown> {
...
function success(
  tool: ToolExecutionInstruction["tool"],
  action: string,
  summary: string,
  output: unknown,
  retryable = false,
...
function unsupported(tool: ToolExecutionInstruction["tool"], action: string): ToolExecutionResult {
  return {
    tool,
    action,
    status: "failed",
...
function isRetryableSmsFailure(detail?: string): boolean {
  if (!detail) {
    return true;
  }
...
function isRetryableEmailFailure(detail?: string): boolean {
  if (!detail) {
    return true;
  }
  const lower = detail.toLowerCase();
...
function readMethod(value: string | undefined): HttpMethod | undefined {
  if (!value) {
    return undefined;
  }
...
function readRecord(source: Record<string, unknown> | undefined, key: string): Record<string, string> | undefined {
  if (!source) {
    return undefined;
  }
  const value = source[key];
...
function readArray(source: Record<string, unknown> | undefined, key: string): readonly unknown[] | undefined {
  if (!source) {
    return undefined;
  }
  const value = source[key];
...
function readArrayOfRecords(source: Record<string, unknown> | undefined, key: string): readonly Record<string, unknown>[] {
