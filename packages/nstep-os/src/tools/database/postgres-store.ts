...
export async function createPostgresDomainStore(pool: Pool): Promise<DomainStore> {
  await ensureSchema(pool);

  return {
    async loadLeads() {
      const result = await pool.query(`SELECT data FROM ${LEADS_TABLE} ORDER BY updated_at ASC`);
      return result.rows.map((row) => row.data as LeadRecord);
    },
    ...
    async getLeadByPhone(tenantId, phone) {
      ...
      const row = result.rows[0] as { data?: LeadRecord } | undefined;
      return row?.data;
    },
    ...
    async listInteractions(tenantId) {
      ...
      return result.rows.map((row) => row.data as LeadInteraction);
    },
...
function inferTenantId(message: SmsMessage): string {
  const raw = (message as SmsMessage & { readonly tenantId?: string }).tenantId;
