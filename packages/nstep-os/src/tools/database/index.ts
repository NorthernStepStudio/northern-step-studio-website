...
interface DomainState {
  ...
  outbounds: SmsMessage[];
  events: LeadRecoveryEvent[];
}
...
function createEmptyState(): DomainState {
  return {
    leads: [],
    interactions: [],
    outbounds: [],
...
async function loadState(filePath: string): Promise<DomainState> {
  return readJsonFile<DomainState>(filePath, createEmptyState());
}

async function saveState(filePath: string, state: DomainState): Promise<void> {
  await writeJsonFile(filePath, state);
}

export async function createJsonDomainStore(options: JsonDomainStoreOptions): Promise<DomainStore> {
  ...
  return {
    ...
    async getLeadByPhone(tenantId, phone) {
      ...
      return state.leads.find(
        (lead) => lead.tenantId === tenantId && normalizePhone(lead.phone) === normalizedPhone,
      );
    },
    ...
    async listOutbounds(tenantId) {
      const state = await loadState(filePath);
      return state.outbounds.filter((item) =>
        state.leads.some((lead) => lead.tenantId === tenantId && normalizePhone(lead.phone) === normalizePhone(item.to)),
      );
    },
...
export function seedLeadRecord(tenantId: string, phone: string, overrides: Partial<LeadRecord> = {}): LeadRecord {
  return {
