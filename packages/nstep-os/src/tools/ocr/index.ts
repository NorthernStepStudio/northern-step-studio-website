...
export function createOcrAdapter(config: OcrAdapterConfig = {}): OcrAdapter {
  const provider = config.endpoint && config.provider !== "mock" ? "generic-http" : "mock";

  return {
    provider,
    async extract(input: OcrExtractionRequest): Promise<OcrExtractionResult> {
...
async function fetchOcrEndpoint(
  endpoint: string,
...
function normalizeEndpointResponse(
  response: unknown,
  sources: readonly OcrExtractionItem[],
  extractedAt: string,
): OcrExtractionResult {
...
function normalizeResultEntry(entry: unknown, source: OcrExtractionItem, index: number): OcrExtractionResultItem {
...
function deriveMockText(item: OcrExtractionItem): string {
  const text = [
    item.text,
...
function deriveMockConfidence(item: OcrExtractionItem): number {
...
function buildSummary(items: readonly OcrExtractionItem[], hasStructuredResponse: boolean): string {
...
function stringFromRecord(record: Record<string, unknown>, ...keys: readonly string[]): string | undefined {
