...
test("knowledge coverage surfaces lane doc counts in health and dashboard", async () => {
  ...
  const chunks = KNOWLEDGE_LANE_SPECS.slice(0, 6).flatMap((spec) =>
  ...
  try {
    ...
    const healthKnowledge = health.knowledge as {
      ...
      readonly coverage: {
        ...
        readonly lanes: readonly { readonly lane: string; readonly present: boolean; readonly chunkCount: number }[];
      };
    };
