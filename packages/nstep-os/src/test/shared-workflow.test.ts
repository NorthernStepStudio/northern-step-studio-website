...
test("shared adapter workflow runs end-to-end through the backend and adapter path", async () => {
  ...
  try {
    ...
    const payload = (await response.json()) as { readonly job: Record<string, any> };
    ...
    assert.deepEqual(
      job.steps.map((step: { readonly status: string }) => step.status),
...
async function startHealthServer(): Promise<{ readonly server: Server; readonly port: number }> {
