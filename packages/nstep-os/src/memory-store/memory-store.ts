...
export async function createJsonMemoryStore(options: JsonMemoryStoreOptions): Promise<MemoryStore> {
  const filePath = path.join(options.dataDir, options.fileName ?? "memory.json");
  await ensureDirectory(options.dataDir);

  const load = async () => readJsonFile<readonly MemoryEntry[]>(filePath, []);
