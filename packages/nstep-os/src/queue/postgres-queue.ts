...
import type { JobQueueEntry, JobQueueStore, JobRecord, JobQueueStatus } from "../core/types.js";
...
export async function createPostgresJobQueueStore(pool: Pool): Promise<JobQueueStore> {
  ...
  return {
    ...
    async get(jobId) {
      ...
      const row = result.rows[0] as { readonly data?: JobQueueEntry } | undefined;
      return row?.data;
    },
    async upsert(entry) {
      await persistEntry(pool, entry);
    ...
    async releaseStaleClaims(staleAfterMs = DEFAULT_DELAY_MS, now = new Date().toISOString()) {
      ...
      const staleEntries = await withClient(pool, async (client) => {
        ...
        return result.rows.map((row) => row.data as JobQueueEntry);
      });
      ...
      for (const entry of staleEntries) {
        const nextEntry = {
          ...entry,
          status: "queued" as JobQueueStatus,
          availableAt: now,
          claimedAt: undefined,
          workerId: undefined,
...
async function readEntries(pool: Pool): Promise<readonly JobQueueEntry[]> {
  const result = await pool.query(`SELECT data FROM ${TABLE_NAME} ORDER BY updated_at ASC`);
  return result.rows.map((row) => row.data as JobQueueEntry);
}

function canClaim(entry: JobQueueEntry | undefined, nowMs: number): boolean {
  if (!entry) {
    return false;
  }
...
async function getEntry(poolOrClient: Pool | PoolClient, jobId: string): Promise<JobQueueEntry | undefined> {
  const result = await poolOrClient.query(`SELECT data FROM ${TABLE_NAME} WHERE job_id = $1 LIMIT 1`, [jobId]);
  const row = result.rows[0] as { readonly data?: JobQueueEntry } | undefined;
  return row?.data;
}

async function updateByJobId(
  pool: Pool,
  jobId: string,
  mapper: (entry: JobQueueEntry) => JobQueueEntry,
...
async function persistEntry(poolOrClient: Pool | PoolClient, entry: JobQueueEntry): Promise<void> {
...
function buildEntry(job: JobRecord, existing: JobQueueEntry | undefined, overrides: Partial<JobQueueEntry>): JobQueueEntry {
  const now = new Date().toISOString();
  return {
    jobId: job.jobId,
...
function priorityOrderSql(): string {
