...
export async function createJsonJobQueueStore(options: JsonJobQueueStoreOptions): Promise<JobQueueStore> {
  const filePath = path.join(options.dataDir, options.fileName ?? "queue.json");
  await ensureDirectory(options.dataDir);

  const load = async () => readJsonFile<readonly JobQueueEntry[]>(filePath, []);
...
function upsertEntry(entries: readonly JobQueueEntry[], entry: JobQueueEntry): JobQueueEntry[] {
...
async function updateEntry(
  filePath: string,
  entries: readonly JobQueueEntry[],
  jobId: string,
...
function compareQueueEntries(left: JobQueueEntry, right: JobQueueEntry): number {
...
function priorityWeight(priority: JobRecord["goal"]["priority"]): number {
  switch (priority) {
    case "critical":
      return 4;
    case "high":
...
function buildQueueMetadata(job: JobRecord, existing?: JobQueueEntry): Record<string, unknown> {
  return {
    ...(existing?.metadata || {}),
    product: job.goal.product,
    goal: job.goal.goal,
