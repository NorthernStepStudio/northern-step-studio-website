...
export function createSharedAdapterWorkflow(): WorkflowDefinition {
  return {
    key: "shared",
    ...
    report(job, context): WorkflowResult {
      const completed = job.steps.every((step) => step.status === "completed");
      const status: WorkflowResult["status"] = job.steps.some((step) => step.status === "failed")
        ? "failed"
        : completed
          ? "succeeded"
          : "partial";
...
function readSharedSample(input: GoalInput | JobRecord["goal"]): {
  ...
  const sample = payload && typeof payload.shared === "object" && payload.shared && !Array.isArray(payload.shared)
    ? (payload.shared as Record<string, unknown>)
    : undefined;
...
function buildVerification(job: JobRecord): VerificationResult {
...
function buildMemoryEntry(
  ...
  entry: {
    ...
    readonly lesson?: MemoryEntry["lesson"];
  },
): MemoryEntry {
