import type {
  ...
  NexusBuildPerformanceSummary,
  NexusBuildPricingSnapshot,
  NexusBuildValueSummary,
  NexusBuildWatchItem,
} from "../../core/types.js";
import {
  estimatePowerDraw,
  getBuildPrice,
  inferMemoryCapacityGb,
  inferPsuMargin,
  inferPsuWattage,
  normalizeText,
  scoreCpuPart,
...
type NexusBuildPart = NexusBuildNormalizedPart;

export function analyzeNexusBuild(
  ...
  const performance: NexusBuildPerformanceSummary = {
  ...
  const value: NexusBuildValueSummary = {
    score: valueScore,
    estimatedBuildCost,
    budgetFit,
    valueNotes,
...
export function compareNexusBuilds(
  builds: readonly NexusBuildBuildSnapshot[],
  baseUseCase: NexusBuildIntake["useCase"],
): NexusBuildComparisonSummary {
...
function analyzeComparisonSnapshot(
  build: NexusBuildBuildSnapshot,
  useCase: NexusBuildIntake["useCase"],
  compatibility: NexusBuildCompatibilitySummary,
): { readonly score: number; readonly summary: string } {
  ...
  const psuScore = scorePowerSupply(psu);
  const performanceScore = clampScore(
    cpuScore * useCaseWeight(useCase, "cpu") +
...
function buildValueNotes(
  intake: NexusBuildIntake,
  estimatedBuildCost: number | undefined,
  budgetFit: number,
...
function buildPricePerformanceNotes(
  intake: NexusBuildIntake,
  parts: readonly NexusBuildPart[],
  estimatedBuildCost: number | undefined,
  performanceScore: number,
  compatibility: NexusBuildCompatibilitySummary,
  pricingSnapshots: readonly NexusBuildPricingSnapshot[],
...
function estimateBudgetFit(budget: number | undefined, cost: number | undefined): number {
  if (budget === undefined) {
...
function scoreUseCaseAlignment(
  useCase: NexusBuildIntake["useCase"],
  cpuScore: number,
  gpuScore: number,
  memoryScore: number,
...
function resolveWeights(useCase: NexusBuildIntake["useCase"]): Record<"cpu" | "gpu" | "memory" | "storage" | "platform" | "compatibility", number> {
  switch (useCase) {
    case "gaming":
...
function useCaseWeight(
  useCase: NexusBuildIntake["useCase"],
  key: "cpu" | "gpu" | "memory" | "storage" | "platform" | "compatibility",
): number {
  return resolveWeights(useCase)[key];
}

function partByCategory(parts: readonly NexusBuildPart[], category: string): NexusBuildPart | undefined {
  return parts.find((part) => part.category === category);
}

function priceShare(parts: readonly NexusBuildPart[], category: string, totalCost: number): number {
  if (totalCost <= 0) {
    return 0;
  }
...
function shareOfPart(parts: readonly NexusBuildPart[], partId: string, totalCost: number): number {
  if (totalCost <= 0) {
    return 0;
  }
...
function normalizePartsText(parts: readonly NexusBuildPart[]): string {
  return parts.map((part) => `${part.name} ${part.brand || ""} ${part.model || ""} ${stringifyValue(part.specs)}`).join(" ");
}

export function summarizeWorkflowWarnings(
  intake: NexusBuildIntake,
  compatibility: NexusBuildCompatibilitySummary,
  ...
  const warnings = new Set<string>(analysis.warnings);
