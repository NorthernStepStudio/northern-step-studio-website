...
import type {
  ...
  NexusBuildIntake,
  NexusBuildIntakePayload,
  NexusBuildNormalizedPart,
  NexusBuildPriceSource,
  NexusBuildUseCase,
  NexusBuildWatchItem,
  NexusBuildWorkflowType,
} from "../../core/types.js";
import {
  DEFAULT_NEXUSBUILD_CURRENCY,
  coerceNumber,
  coerceRecord,
  coerceString,
  inferBrand,
...
interface NexusBuildPayloadEnvelope {
  readonly nexusbuild?: unknown;
  readonly nexusBuild?: unknown;
}

const OPERATION_KEYWORDS: Array<[NexusBuildWorkflowType, RegExp]> = [
...
export function extractNexusBuildIntake(goal: GoalInput): NexusBuildIntake {
  ...
  const currentParts = normalizePartCollection(
    raw.parts ??
      raw.components ??
      buildRecord ??
...
function extractRawNexusBuildPayload(payload: Record<string, unknown>): Record<string, any> {
  const envelope = payload as NexusBuildPayloadEnvelope;
  return coerceRecord(envelope.nexusbuild) || coerceRecord(envelope.nexusBuild) || payload;
}

function resolveOperation(goalText: string, raw: Record<string, unknown>): NexusBuildWorkflowType {
...
function resolveUseCase(goalText: string, raw: Record<string, unknown>): NexusBuildUseCase {
...
function resolveBuildName(goalText: string, raw: Record<string, unknown>, useCase: NexusBuildUseCase, operation: NexusBuildWorkflowType): string {
...
function normalizeSavedBuilds(
  raw: unknown,
  currentSnapshot: NexusBuildBuildSnapshot,
  tenantId: string,
  name: string,
  useCase: NexusBuildUseCase,
  budget: number | undefined,
  currency: string,
  fallbackParts: readonly NexusBuildNormalizedPart[],
): NexusBuildBuildSnapshot[] {
...
function normalizeComparisonBuilds(raw: unknown, currentBuildId: string, tenantId: string): NexusBuildBuildSnapshot[] {
  return flattenCollection(raw)
    .map((item, index) =>
      normalizeBuildSnapshot(item, {
...
function normalizePriceSources(raw: unknown): NexusBuildPriceSource[] {
  return flattenCollection(raw)
    ...
    .map((item) => ({
      ...
      kind:
        ...
        coerceString(item.kind) === "review"
          ? (coerceString(item.kind) as NexusBuildPriceSource["kind"])
          : undefined,
...
function normalizeWatchlist(raw: unknown): NexusBuildWatchItem[] {
  return flattenCollection(raw)
...
function resolveLivePricingEnabled(
  rawMode: unknown,
  priceSources: readonly NexusBuildPriceSource[],
...
function normalizePartCollection(raw: unknown, source: NexusBuildNormalizedPart["source"]): NexusBuildNormalizedPart[] {
  const candidates = flattenPartCandidates(raw);
  return candidates
    .map((candidate, index) => normalizePartCandidate(candidate, source, index))
    .filter((part): part is NexusBuildNormalizedPart => Boolean(part));
}

function normalizePartCandidate(
  candidate: unknown,
  source: NexusBuildNormalizedPart["source"],
  index: number,
): NexusBuildNormalizedPart | undefined {
...
function extractSpecs(record: Record<string, unknown>): Record<string, unknown> {
...
function flattenPartCandidates(raw: unknown): unknown[] {
  if (!raw) {
    return [];
  }
...
function isPartCategoryKey(key: string): boolean {
  return [
    "cpu",
    "motherboard",
    "gpu",
...
function parseCandidatePrice(value: unknown): number | undefined {
...
function isNexusBuildOperation(value: string): value is NexusBuildWorkflowType {
  return [
    "build-intake",
    ...
    "parts-comparison",
  ].includes(value as NexusBuildWorkflowType);
}
