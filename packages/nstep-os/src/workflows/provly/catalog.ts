...
const CATEGORY_KEYWORDS: Array<[ProvLyItemCategory, string[]]> = [
...
const HIGH_VALUE_CATEGORIES = new Set<ProvLyItemCategory>(["electronics", "jewelry", "art", "collectibles"]);
...
export function buildCategoryId(label: string): string {
  return `category_${slugify(label)}`;
}

export function buildRoomId(label: string): string {
  return `room_${slugify(label)}`;
}

export function inferCategory(text: unknown, fallbackCategory?: unknown): ProvLyItemCategory {
...
export function inferRoomLabel(text: unknown, fallback?: string): string {
...
export function inferCondition(text: unknown): ProvLyItemCondition {
...
export function inferHighValue(category: ProvLyItemCategory, estimatedValue: number | undefined, threshold = DEFAULT_PROVLY_HIGH_VALUE_THRESHOLD, text?: unknown): boolean {
...
export function inferBrand(text: unknown): string | undefined {
  const normalized = normalizeText(text);
  if (!normalized) {
    return undefined;
  }

  const brandMatchers: Array<[RegExp, string]> = [
...
export function formatMoney(amount: number | undefined, currency = DEFAULT_PROVLY_CURRENCY): string {
