...
export function normalizeProvLyInventory(intake: ProvLyIntake): ProvLyNormalizationResult {
...
function normalizeInventoryItems(
  rawItems: readonly ProvLyInventoryItem[] | readonly unknown[],
  intake: ProvLyIntake,
  highValueThreshold: number,
): ProvLyInventoryItem[] {
  const candidates = flattenCollection(rawItems);
  return candidates.map((candidate, index) => normalizeItem(candidate, intake, highValueThreshold, index)).filter(Boolean) as ProvLyInventoryItem[];
}

function normalizeItem(candidate: unknown, intake: ProvLyIntake, highValueThreshold: number, index: number): ProvLyInventoryItem | undefined {
  ...
  const estimatedValue =
    coerceNumber(record?.estimatedValue) ??
...
function normalizeAttachments(rawAttachments: readonly unknown[], intake: ProvLyIntake): ProvLyAttachment[] {
  return flattenCollection(rawAttachments).map((candidate, index) => normalizeAttachment(candidate, intake, index)).filter(Boolean) as ProvLyAttachment[];
}

function normalizeAttachment(candidate: unknown, intake: ProvLyIntake, index: number): ProvLyAttachment | undefined {
  const record = coerceRecord(candidate);
  if (!record && candidate === undefined) {
    return undefined;
  }
...
function normalizeReceipts(rawReceipts: readonly unknown[], intake: ProvLyIntake): ProvLyReceipt[] {
  return flattenCollection(rawReceipts).map((candidate, index) => normalizeReceipt(candidate, intake, index)).filter(Boolean) as ProvLyReceipt[];
}

function normalizeReceipt(candidate: unknown, intake: ProvLyIntake, index: number): ProvLyReceipt | undefined {
  const record = coerceRecord(candidate);
  if (!record && candidate === undefined) {
    return undefined;
  }
...
function collectRoomLabels(intake: ProvLyIntake, items: readonly ProvLyInventoryItem[]): string[] {
  const labels = new Set<string>(intake.rooms.map((room) => room.trim()).filter(Boolean));
...
function resolveItemSource(value: unknown): ProvLyItemSource {
...
function resolveAttachmentKind(value: unknown): ProvLyAttachmentKind {
...
function buildInferredFields(
  ...
  values: {
    ...
    readonly category: ProvLyItemCategory;
...
function countSources(items: readonly ProvLyInventoryItem[]): Record<ProvLyItemSource, number> {
  const counts: Record<ProvLyItemSource, number> = {
    manual: 0,
    upload: 0,
    photo: 0,
    receipt: 0,
