...
import type { OcrAdapter, OcrExtractionItem, OcrExtractionResultItem } from "../../tools/ocr/index.js";
import type {
  ProvLyAttachment,
  ProvLyIntake,
  ProvLyInventoryItem,
...
import {
  ...
  DEFAULT_PROVLY_UNASSIGNED_ROOM,
  buildCategoryId,
  buildRoomId,
  coerceNumber,
  coerceRecord,
...
interface VisualCandidate {
  readonly id: string;
  readonly kind: "photo" | "receipt" | "document" | "other";
...
export async function enrichProvLyIntakeFromVisualAssets(intake: ProvLyIntake, ocr?: OcrAdapter): Promise<ProvLyVisualExtractionResult> {
  ...
  const generatedItems: ProvLyInventoryItem[] = [];
  const generatedReceipts: ProvLyReceipt[] = [];
  const generatedAttachments: ProvLyAttachment[] = [];
  ...
  for (const candidate of candidates) {
    ...
    if (looksLikeReceipt(candidate, text)) {
      generatedReceipts.push(
        buildReceiptDraft(intake, candidate, attachment.attachmentId, text, result?.confidence),
      );
    ...
    generatedItems.push(
      buildItemDraft(intake, candidate, attachment.attachmentId, text, result?.confidence),
    );
    notes.add(`Extracted inventory details from ${candidate.label || candidate.filename || candidate.id}.`);
  }

  const enrichedIntake: ProvLyIntake = {
    ...intake,
    ...
    claimContext: {
      ...
      visualExtraction: {
        ...
        notes: [...notes],
        extractedAt: new Date().toISOString(),
      } satisfies ProvLyVisualExtractionSummary,
    },
  };

  const summary = enrichedIntake.claimContext.visualExtraction as ProvLyVisualExtractionSummary;
  return {
...
async function runOcr(candidates: readonly VisualCandidate[], ocr?: OcrAdapter): Promise<readonly OcrExtractionResultItem[]> {
  if (!ocr || candidates.length === 0) {
    return [];
  }

  const items: OcrExtractionItem[] = candidates
  ...
  try {
    const response = await ocr.extract({
      items,
      prompt:
        "Extract concise text from home inventory photos and receipts. Preserve item names, brands, model numbers, receipt totals, vendor names, dates, room clues, and serial numbers when visible.",
    });
...
function resolveExtractionText(candidate: VisualCandidate, result?: OcrExtractionResultItem): string {
  return (
    result?.text ||
    candidate.text ||
...
function collectVisualCandidates(intake: ProvLyIntake): VisualCandidate[] {
  const candidates = new Map<string, VisualCandidate>();
...
function normalizeCandidate(raw: unknown, index: number): VisualCandidate | undefined {
  ...
  const id =
    (record && (coerceString(record.attachmentId) || coerceString(record.receiptId) || coerceString(record.itemId))) ||
...
function buildAttachment(intake: ProvLyIntake, candidate: VisualCandidate, text: string, result?: OcrExtractionResultItem): ProvLyAttachment {
  return {
...
function buildItemDraft(
  ...
  const categoryLabel = titleize(
    coerceString(candidate.metadata.categoryLabel) ||
  ...
  const roomLabel =
    titleize(
      coerceString(candidate.metadata.roomLabel) ||
        coerceString(candidate.metadata.room) ||
        inferRoomLabel(`${text} ${name}`, intake.claimType || DEFAULT_PROVLY_UNASSIGNED_ROOM),
    ) || DEFAULT_PROVLY_UNASSIGNED_ROOM;
...
function annotateIntakeWithExtraction(intake: ProvLyIntake, summary: ProvLyVisualExtractionSummary): ProvLyIntake {
  return {
    ...intake,
    claimContext: {
      ...intake.claimContext,
...
function resolveItemName(candidate: VisualCandidate, text: string): string {
  const firstLine = text
    .split(/\r?\n/)
    .map((line) => line.trim())
    .find(Boolean);
  return (
    coerceString(candidate.metadata.itemName) ||
...
function stripReceiptPhrases(value: string): string {
  return titleize(
    value
...
function looksLikeReceipt(candidate: VisualCandidate, text: string): boolean {
...
function attachmentFingerprint(attachment: Pick<ProvLyAttachment, "filename" | "url" | "label" | "mimeType">): string {
  return [attachment.url, attachment.filename, attachment.label, attachment.mimeType].filter(Boolean).join("|");
}

function attachmentFingerprintFromValue(value: unknown): string {
...
function mergeUnique<T>(base: readonly T[], additions: readonly T[], getId: (value: T) => string): T[] {
  const seen = new Set(base.map(getId));
  return [...base, ...additions.filter((item) => !seen.has(getId(item)))];
}

function stringFromMetadata(record: Record<string, unknown>, ...keys: readonly string[]): string | undefined {
