...
export function buildProvLyAnalysisReport(context: ProvLyReportContext): ProvLyAnalysisReport {
...
export function buildProvLyWorkflowResult(context: ProvLyReportContext, report: ProvLyAnalysisReport): WorkflowResult {
...
export function buildProvLyActions(context: Pick<ProvLyReportContext, "reminderResult">): string[] {
  const actions = [
...
function buildSummary(context: ProvLyReportContext): string {
  const completenessLine =
    context.completeness.claimReady && context.completeness.status === "pass"
  ...
  const extractionLine =
    extraction && extraction.extractedItemCount + extraction.extractedReceiptCount > 0
...
function buildWarnings(context: ProvLyReportContext): string[] {
...
function buildReminders(context: ProvLyReportContext): string[] {
  return [...new Set([...context.completeness.reminders, ...(context.reminderResult?.status === "draft" ? ["Reminder draft prepared for follow-up."] : [])])];
}

function resolveStatus(context: ProvLyReportContext, report: ProvLyAnalysisReport): WorkflowResult["status"] {
...
function buildDashboardSummary(context: ProvLyReportContext, report: ProvLyAnalysisReport): Record<string, unknown> {
  ...
  return {
    ...
    visualExtraction,
    highValueItems: context.classification.highValueItems.slice(0, 10).map((item: ProvLyInventoryItem) => ({
      itemId: item.itemId,
      name: item.name,
...
function resolveVisualExtractionSummary(context: ProvLyReportContext):
  | {
...
function buildReminderActionNotes(context: ProvLyReportContext): string[] {
  if (context.reminderResult?.status === "sent") {
