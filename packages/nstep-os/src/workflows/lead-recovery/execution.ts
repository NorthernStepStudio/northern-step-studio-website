...
import type { SmsAdapter } from "../../tools/sms/index.js";
...
export async function executeLeadRecoveryStep(step: JobStepState, context: WorkflowExecutionContext & { readonly input: LeadRecoveryInput }): Promise<StepResult> {
  const input = context.input;
  const assessment = buildLeadRecoveryAssessment(input);
  const database = context.stores.domain;
  const sms = context.tools.sms as SmsAdapter | undefined;
  const lead = input.lead || (await database.getLeadByPhone(input.goal.tenantId, input.event.callerPhone)) || buildLeadRecord(input);
  const getHistory = (): LeadRecoveryHistorySnapshot =>
    readLeadRecoveryHistorySnapshot(context.job.steps.find((item) => item.type === STEP_TYPES.loadLead)) ||
    createEmptyLeadRecoveryHistory(input, lead);
  const getDecision = (): LeadRecoveryDecision => buildLeadRecoveryDecision(input, assessment, getHistory());
  ...
  switch (step.type) {
    ...
    case STEP_TYPES.send: {
      ...
      const drafted = (draftStep?.result?.output as
        | {
            readonly draft?: LeadRecoveryMessageDraft;
            readonly decision?: LeadRecoveryDecision;
            readonly safety?: LeadRecoveryDraftSafety;
          }
        | undefined) || undefined;
    ...
    case STEP_TYPES.verify: {
      ...
      const sendOutput = sendStep?.result?.output as
        | {
            readonly delivery?: {
              readonly messageId?: string;
              readonly provider?: string;
              readonly status?: NonNullable<SmsMessage["status"]>;
      ...
      const delivery = sms && messageId ? await sms.verify(messageId) : { status: "unknown" as const };
      return {
        status: "completed",
    ...
    case STEP_TYPES.log: {
      ...
      const sendOutput = sendStep?.result?.output as
        ...
        | undefined;
      const sendDelivery = (sendOutput?.delivery as
        | {
            readonly messageId?: string;
            readonly provider?: string;
            readonly status?: NonNullable<SmsMessage["status"]>;
      ...
      const deliveryStatus: NonNullable<SmsMessage["status"]> = sendDelivery?.status || "unknown";
      const messageId = sendDelivery?.messageId || `sms_${randomUUID()}`;
      const outboundStatus: NonNullable<SmsMessage["status"]> =
      ...
      const interaction: LeadInteraction = {
        ...
        metadata: {
          ...
          safety: (draftStep?.result?.output as { readonly safety?: LeadRecoveryDraftSafety } | undefined)?.safety,
          deliveryStatus,
          messageId,
          outboundStatus,
        },
      ...
      await database.appendOutboundMessage({
        to: input.event.callerPhone,
        from: input.brand.smsFromNumber,
        body: (draftStep?.result?.output as { readonly draft?: LeadRecoveryMessageDraft } | undefined)?.draft?.body || buildLeadRecoveryMessage(input, assessment, decision.scenario).body,
        tenantId: input.goal.tenantId,
