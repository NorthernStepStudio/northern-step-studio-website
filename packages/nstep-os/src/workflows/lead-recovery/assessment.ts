import type { LeadInteraction, LeadRecord, LeadRecoveryAssessment, LeadRecoveryBrandProfile, LeadRecoveryEvent, LeadRecoveryHistorySnapshot, LeadRecoveryInput, LeadRecoveryScenario, WorkflowExecutionContext, JobStepState } from "../../core/types.js";
...
export function buildLeadRecoveryAssessment(input: LeadRecoveryInput): LeadRecoveryAssessment {
  const now = Date.now();
  const previousInteractionTimes =
  ...
  const contactedRecently =
    (Number.isFinite(lastOutboundAt) && now - lastOutboundAt < 48 * 60 * 60 * 1000) ||
...
export async function loadLeadRecoveryHistorySnapshot(
  database: WorkflowExecutionContext["stores"]["domain"],
  input: LeadRecoveryInput,
  lead: LeadRecord,
): Promise<LeadRecoveryHistorySnapshot> {
  ...
  const leadInteractions = interactions.filter(
    (interaction) =>
      interaction.leadId === lead.leadId ||
...
export function readLeadRecoveryHistorySnapshot(step: JobStepState | undefined): LeadRecoveryHistorySnapshot | undefined {
  const output = step?.result?.output as { readonly history?: LeadRecoveryHistorySnapshot } | undefined;
  return output?.history;
}

export function buildLeadRecoveryDecision(
  input: LeadRecoveryInput,
  assessment: LeadRecoveryAssessment,
  history: LeadRecoveryHistorySnapshot | undefined,
): LeadRecoveryDecision {
  const safeHistory: LeadRecoveryHistorySnapshot = history || createEmptyLeadRecoveryHistory(input, input.lead || buildLeadRecord(input));
  ...
  const contactedRecently =
    (Number.isFinite(recentOutboundAt) && Date.now() - recentOutboundAt < 48 * 60 * 60 * 1000) ||
    (Number.isFinite(recentInteractionAt) && Date.now() - recentInteractionAt < 48 * 60 * 60 * 1000) ||
    Boolean(input.lead?.contactedWithin48h);
  const complianceFlags = new Set<string>(assessment.complianceFlags);
...
export function determineLeadRecoveryScenario(
  input: LeadRecoveryInput,
  assessment: LeadRecoveryAssessment,
  history: LeadRecoveryHistorySnapshot,
): LeadRecoveryScenario {
  const text = [
    input.goal.goal,
...
export function createEmptyLeadRecoveryHistory(input: LeadRecoveryInput, lead: LeadRecord): LeadRecoveryHistorySnapshot {
  return {
    leadId: lead.leadId,
    tenantId: input.goal.tenantId,
    interactionCount: 0,
...
function hasOptOutSignal(interaction: LeadInteraction): boolean {
...
function collectTextValues(value: unknown, collected: string[] = [], depth = 0): string[] {
  if (depth > 4) {
    return collected;
  }
...
function isWithinBusinessHours(brand: LeadRecoveryBrandProfile, atMs: number): boolean {
  ...
  return (
    brand.businessHours.days.map((day) => day.toLowerCase()).includes(weekday) &&
    time >= brand.businessHours.open &&
    time <= brand.businessHours.close
  );
}

function isUrgent(event: LeadRecoveryEvent, lead?: LeadRecord): boolean {
