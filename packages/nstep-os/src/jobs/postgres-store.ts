...
export async function createPostgresJobStore(pool: Pool): Promise<JobStore> {
  ...
  return {
    ...
    async get(jobId) {
      ...
      const row = result.rows[0] as { data?: JobRecord } | undefined;
      return row?.data;
    },
    async upsert(job) {
      await pool.query(
...
async function readJobs(pool: Pool): Promise<readonly JobRecord[]> {
