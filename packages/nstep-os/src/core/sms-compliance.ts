...
export function containsSmsOptOutKeyword(text: string): boolean {
  return SMS_OPT_OUT_PATTERN.test(normalizeSmsBody(text));
}

export function buildSmsCompliantBody(body: string, maxCharacters = SMS_MAX_CHARACTERS): string {
...
export function smsPrefix(body: string, length = SMS_REPEAT_PREFIX_CHARACTERS): string {
  return normalizeSmsBody(body).slice(0, length).toLowerCase();
}

export function hasRepeatedSmsPrefix(previousBody: string | undefined, body: string, length = SMS_REPEAT_PREFIX_CHARACTERS): boolean {
  if (!previousBody) {
    return false;
  }
