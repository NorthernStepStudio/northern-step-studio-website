...
import type { Stage2AgentId, Stage2RuntimePhase } from "../core/stage2-models.js";
import type { Stage2InvocationRecord } from "../core/stage2-models.js";
import type {
  DashboardActionPreview,
  DashboardAlert,
  DashboardApprovalQueueItem,
  DashboardApprovalAuditEntry,
  DashboardApprovalQueueResponse,
  DashboardApprovalQueueSummary,
  DashboardEnvelope,
  DashboardJobDetailRecord,
  DashboardJobDetailResponse,
  DashboardJobListItem,
  DashboardJobListResponse,
  DashboardJobListSummary,
  DashboardJobSurface,
  DashboardLogEntry,
  DashboardLogFeedResponse,
  DashboardLogFeedSummary,
  DashboardMemoryAuditEntry,
  DashboardMemoryItem,
  DashboardMemoryViewResponse,
  DashboardMemoryViewSummary,
  DashboardMetric,
  ...
  DashboardQuery,
  DashboardRecurringJobItem,
  DashboardRetryView,
  DashboardStepTimelineItem,
  DashboardWorkflowActivityProductItem,
  DashboardWorkflowActivityResponse,
  DashboardWorkflowActivitySummary,
  DashboardLeadRecoveryPanelResponse,
  DashboardLeadRecoveryResultItem,
  DashboardLeadRecoveryResultSummary,
  DashboardNexusBuildPanelResponse,
  DashboardProvLyPanelResponse,
  DashboardNeuroMovesPanelResponse,
  DashboardSettingsCommunicationTemplate,
  DashboardSettingsOrchestrationHistory,
  DashboardSettingsOrchestrationHistoryItem,
  DashboardSettingsOrchestrationHistorySummary,
  DashboardSettingsResponse,
  DashboardSettingsRuntimeSummary,
  DashboardSettingsSuppressionRule,
  DashboardSettingsSummary,
  DashboardSharedWorkflowOverview,
  DashboardSharedWorkflowSummary,
} from "./contracts.js";
...
export function createDashboardService(
  ...
  orchestrator?: Stage2RuntimeOrchestrator,
): DashboardService {
  return {
    ...
    async panel(product: ProductKey, query = {}) {
      return buildProductPanel(stores, product, query);
    },
  };
}

const PRODUCT_ORDER: readonly ProductKey[] = ["lead-recovery", "nexusbuild", "provly", "neurormoves"];
const ACTIVE_JOB_STATUSES: readonly JobStatus[] = ["pending", "queued", "routing", "planning", "waiting_approval", "running", "verifying"];
...
async function buildProductPanels(stores: RuntimeStores, query: DashboardQuery): Promise<readonly DashboardProductPanelResponse[]> {
  if (query.product) {
...
function buildSharedWorkflowOverview(
...
async function buildProductPanel(stores: RuntimeStores, product: ProductKey, query: DashboardQuery): Promise<DashboardProductPanelResponse> {
  switch (product) {
    case "lead-recovery":
...
function filterJobs(jobs: readonly JobRecord[], query: DashboardQuery): readonly JobRecord[] {
...
function filterMemory(memory: readonly MemoryEntry[], query: DashboardQuery): readonly MemoryEntry[] {
...
function mapJobsForList(jobs: readonly JobRecord[], memory: readonly MemoryEntry[] = []): readonly DashboardJobListItem[] {
  const memoryJobIds = new Set(memory.map((entry) => entry.sourceJobId).filter((item): item is string => Boolean(item)));
...
function mapJobForDetail(job: JobRecord, memory: readonly MemoryEntry[] = []): DashboardJobDetailRecord {
      return {
        ...buildJobSurface(job),
...
function mapJobTimeline(job: JobRecord): readonly DashboardStepTimelineItem[] {
  return [...job.steps]
...
function mapJobLogs(job: JobRecord): readonly DashboardLogEntry[] {
  return [...job.logs]
    .sort((left, right) => right.at.localeCompare(left.at))
    .map((entry) => ({
      id: entry.id,
...
function buildApprovalQueueItems(jobs: readonly JobRecord[]): readonly DashboardApprovalQueueItem[] {
  const items: DashboardApprovalQueueItem[] = [];
...
function buildApprovalAuditTrail(job: JobRecord, step: JobStepState): readonly DashboardApprovalAuditEntry[] {
  const approvals: DashboardApprovalAuditEntry[] = [];
...
function resolveApprovalAuditAction(log: JobRecord["logs"][number]): DashboardApprovalAuditEntry["action"] | undefined {
...
function buildLogFeedItems(jobs: readonly JobRecord[], query: DashboardQuery = {}): readonly DashboardLogEntry[] {
  const items: DashboardLogEntry[] = [];
  for (const job of jobs) {
    if (query.jobId && job.jobId !== query.jobId) {
      continue;
    }
...
function summarizeJobs(jobs: readonly JobRecord[]): DashboardJobListSummary {
...
function summarizeSharedWorkflow(jobs: readonly JobRecord[]): DashboardSharedWorkflowSummary {
  let running = 0;
...
function summarizeApprovals(items: readonly DashboardApprovalQueueItem[]): DashboardApprovalQueueSummary {
  const byProduct = createProductCountMap();
  const byWorkflow: Record<string, number> = {};
...
function summarizeLogs(items: readonly DashboardLogEntry[]): DashboardLogFeedSummary {
  const byProduct = createProductCountMap();
  const byWorkflow: Record<string, number> = {};
  let debug = 0;
  let info = 0;
...
function summarizeMemory(memory: readonly MemoryEntry[]): DashboardMemoryViewSummary {
...
function summarizeMemoryByCategory(memory: readonly MemoryEntry[]): Record<string, number> {
  return memory.reduce<Record<string, number>>((accumulator, entry) => {
    accumulator[entry.category] = (accumulator[entry.category] || 0) + 1;
    return accumulator;
  }, {});
}

function summarizeMemoryByProduct(memory: readonly MemoryEntry[]): Record<ProductKey, number> {
...
function buildWorkflowActivitySummary(jobs: readonly JobRecord[]): DashboardWorkflowActivitySummary {
...
function buildWorkflowActivityResponse(
  jobs: readonly JobRecord[],
  panels: readonly DashboardProductPanelResponse[],
...
function buildOverviewAlerts(
  jobs: readonly JobRecord[],
  recentMemory: readonly DashboardMemoryItem[],
  panels: readonly DashboardProductPanelResponse[],
  knowledgeCoverage: DashboardSnapshot["knowledge"],
...
function buildProductCard(panel: DashboardProductPanelResponse, jobs: readonly JobRecord[]): DashboardProductCard {
  const productJobs = jobs.filter((job) => job.goal.product === panel.product);
  ...
  const lastActivityAt = productJobs.reduce<string | undefined>((latest, job) => (!latest || job.updatedAt > latest ? job.updatedAt : latest), panel.recentJobs[0]?.updatedAt);
...
function buildRecurringJobsForProduct(jobs: readonly JobRecord[], product?: ProductKey): readonly DashboardRecurringJobItem[] {
  return [...jobs]
...
function buildWorkflowActivityProductItem(panel: DashboardProductPanelResponse, jobs: readonly JobRecord[]): DashboardWorkflowActivityProductItem {
...
function buildActionPreview(step: JobRecord["steps"][number], job: JobRecord): DashboardActionPreview {
  return {
    title: step.title,
...
function buildRetryView(retry: { readonly attempts: number; readonly maxAttempts: number; readonly retryable: boolean; readonly exhausted: boolean; readonly lastAttemptAt?: string; readonly lastError?: string; readonly nextRetryAt?: string; }): DashboardRetryView {
  return {
...
function buildJobSurface(job: JobRecord): DashboardJobSurface {
...
function buildJobTags(job: JobRecord): readonly string[] {
...
function buildApprovalPreview(job: JobRecord): DashboardActionPreview | undefined {
...
function currentJobStep(job: JobRecord): JobRecord["steps"][number] | undefined {
  return (
...
function currentApprovalStep(job: JobRecord): JobRecord["steps"][number] | undefined {
  return job.steps.find((step) => step.status === "waiting_approval" || step.approvalRequired) || currentJobStep(job);
}

function createJobStatusCountMap(): Record<JobStatus, number> {
  return {
    pending: 0,
    queued: 0,
    routing: 0,
...
function createApprovalStatusCountMap(): Record<ApprovalStatus, number> {
  return {
    not_required: 0,
    pending: 0,
    approved: 0,
...
function createProductCountMap(): Record<ProductKey, number> {
  return {
    "lead-recovery": 0,
    nexusbuild: 0,
    provly: 0,
...
function createLaneCountMap(): Record<ExecutionLane, number> {
  return {
    internal: 0,
    external: 0,
    mixed: 0,
...
function buildPageInfo(total: number, page: number, pageSize: number): DashboardPageInfo {
...
function compareJobsByRecentUpdate(left: JobRecord, right: JobRecord): number {
...
function sortJobs(
  ...
  sortDirection: DashboardQuery["sortDirection"],
): readonly JobRecord[] {
...
function compareJobFields(left: JobRecord, right: JobRecord, sortBy?: DashboardQuery["sortBy"]): number {
  switch (sortBy) {
    case "createdAt":
...
function compareMemoryFields(left: MemoryEntry, right: MemoryEntry, sortBy?: DashboardQuery["sortBy"]): number {
  switch (sortBy) {
    case "createdAt":
      return left.createdAt.localeCompare(right.createdAt);
    case "status":
...
function extractJobSearchText(job: JobRecord): string {
...
function extractMemorySearchText(entry: MemoryEntry): string {
  const value = typeof entry.value === "string" ? entry.value : JSON.stringify(entry.value);
  return [entry.id, entry.key, entry.category, entry.product, value].join(" ").toLowerCase();
}

function mapMemoryItems(memory: readonly MemoryEntry[]): readonly DashboardMemoryItem[] {
  return [...memory]
...
function buildMemoryAuditTrail(memory: readonly MemoryEntry[]): readonly DashboardMemoryAuditEntry[] {
  const explicitAuditEntries = memory.flatMap((entry) =>
...
function isPatternCategory(category: string): boolean {
...
async function buildLeadRecoveryPanel(stores: RuntimeStores, query: DashboardQuery): Promise<DashboardLeadRecoveryPanelResponse> {
...
function buildLeadRecoveryResults(jobs: readonly JobRecord[]): readonly DashboardLeadRecoveryResultItem[] {
  return [...jobs]
    .sort(compareJobsByRecentUpdate)
    ...
    .map((job) => {
      ...
      return {
        ...
        leadStage: (resolveText(data.leadStage) as DashboardLeadRecoveryResultItem["leadStage"]) || lead?.stage || "new",
...
function buildLeadRecoveryResultSummary(
  results: readonly DashboardLeadRecoveryResultItem[],
...
function extractLeadRecoveryLead(job: JobRecord): { readonly leadId?: string; readonly name?: string; readonly phone?: string; readonly stage?: string } | undefined {
  const payload = job.goal.payload;
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return undefined;
  }

  const leadRecovery = (payload as {
    readonly leadRecovery?: {
      readonly lead?: { readonly leadId?: string; readonly name?: string; readonly phone?: string; readonly stage?: string };
    };
  }).leadRecovery;
...
function extractLeadRecoveryCallEvents(jobs: readonly JobRecord[]): readonly {
  eventId: string;
  callerPhone: string;
  calledNumber: string;
  missedAt: string;
  ...
  for (const job of jobs) {
    const leadRecovery = job.goal.payload && typeof job.goal.payload === "object"
      ? (job.goal.payload as { readonly leadRecovery?: { readonly event?: Record<string, unknown> } }).leadRecovery
      : undefined;
...
function extractLeadRecoveryTemplates(memory: readonly MemoryEntry[]): readonly {
  scenario: "generic-callback" | "after-hours" | "service-inquiry" | "quote-followup" | "appointment-callback";
  title: string;
  body: string;
  ...
  return mapMemoryItems(memory)
    ...
    .map((entry) => {
      const value = typeof entry.value === "object" && entry.value ? (entry.value as Record<string, unknown>) : undefined;
...
function normalizeLeadRecoveryScenario(value: unknown): "generic-callback" | "after-hours" | "service-inquiry" | "quote-followup" | "appointment-callback" {
...
function normalizeLeadRecoveryTone(value: unknown): "business-safe" | "warm" | "urgent" {
...
function humanizeScenarioLabel(scenario: "generic-callback" | "after-hours" | "service-inquiry" | "quote-followup" | "appointment-callback"): string {
  switch (scenario) {
    case "after-hours":
...
function compareLeadRecency(
  left: { readonly lastContactedAt?: string; readonly lastOutboundAt?: string; readonly lastInboundAt?: string },
  right: { readonly lastContactedAt?: string; readonly lastOutboundAt?: string; readonly lastInboundAt?: string },
): number {
  return getLeadRecency(right) - getLeadRecency(left);
}

function getLeadRecency(lead: { readonly lastContactedAt?: string; readonly lastOutboundAt?: string; readonly lastInboundAt?: string }): number {
...
async function buildNexusBuildPanel(stores: RuntimeStores, query: DashboardQuery): Promise<DashboardNexusBuildPanelResponse> {
  const tenantId = query.tenantId || "default";
...
function buildNexusBuildAlerts(input: {
  ...
  readonly snapshots: readonly { readonly price?: number; readonly source: string }[];
  readonly productJobs: readonly JobRecord[];
}): readonly DashboardAlert[] {
  const alerts: DashboardAlert[] = [];
...
function compareReportByRecentUpdate(left: { readonly updatedAt: string }, right: { readonly updatedAt: string }): number {
  return right.updatedAt.localeCompare(left.updatedAt);
}

function compareRunByRecentUpdate(left: { readonly updatedAt: string }, right: { readonly updatedAt: string }): number {
  return right.updatedAt.localeCompare(left.updatedAt);
}

function filterNexusBuildReports(
  reports: readonly NexusBuildAnalysisReport[],
  buildId?: string,
): readonly NexusBuildAnalysisReport[] {
...
function filterNexusBuildChecks(
  checks: readonly NexusBuildCompatibilityCheck[],
  buildId?: string,
): readonly NexusBuildCompatibilityCheck[] {
  if (!buildId) {
    return checks;
  }
...
function filterNexusBuildRuns(
  runs: readonly NexusBuildRecommendationRun[],
  buildId?: string,
): readonly NexusBuildRecommendationRun[] {
  if (!buildId) {
    return runs;
  }
...
function filterNexusBuildSnapshots(
  snapshots: readonly NexusBuildPricingSnapshot[],
  buildId?: string,
): readonly NexusBuildPricingSnapshot[] {
  if (!buildId) {
    return snapshots;
  }
...
function buildFilteredNexusBuildReport(
  report: NexusBuildAnalysisReport,
  priceSource?: string,
  comparisonBuildId?: string,
): NexusBuildAnalysisReport {
...
function filterNexusBuildComparisonMatrix(
  matrix: readonly { readonly buildId: string; readonly score: number; readonly summary: string }[],
  comparisonBuildId?: string,
...
async function buildProvLyPanel(stores: RuntimeStores, query: DashboardQuery): Promise<DashboardProvLyPanelResponse> {
  const tenantId = query.tenantId || "default";
...
function buildProvLyAlerts(input: {
  readonly latestCheck?: { readonly score: number; readonly claimReady: boolean; readonly missingFields: readonly string[]; };
  readonly readyExportCount: number;
  readonly items: readonly { readonly highValue: boolean; }[];
}): readonly DashboardAlert[] {
  const alerts: DashboardAlert[] = [];
...
function lookupProvLyMissingFields(
  item: { readonly itemId: string; readonly receiptIds: readonly string[]; readonly attachmentIds: readonly string[]; readonly purchaseDate?: string; readonly estimatedValue?: number; readonly serialNumber?: string; },
  latestCheck?: { readonly itemScores: readonly { readonly itemId: string; readonly missingFields: readonly string[]; }[] },
): readonly string[] {
...
function collectProvLyReminders(
  items: readonly { readonly name: string; readonly highValue: boolean; readonly receiptIds: readonly string[]; readonly attachmentIds: readonly string[]; }[],
  latestCheck?: { readonly reminders: readonly string[] },
): readonly string[] {
  const reminders = new Set<string>(latestCheck?.reminders || []);
  for (const item of items) {
...
function sumEstimatedValue(items: readonly { readonly estimatedValue?: number }[]): number | undefined {
  const values = items
...
async function buildNeuroMovesPanel(stores: RuntimeStores, query: DashboardQuery): Promise<DashboardNeuroMovesPanelResponse> {
  const tenantId = query.tenantId || "default";
  const [jobs, memory] = await Promise.all([stores.jobs.list(), stores.memory.list()]);
...
async function buildDashboardSettings(
  stores: RuntimeStores,
  config: RuntimeConfig,
  query: DashboardQuery,
  orchestrator?: Stage2RuntimeOrchestrator,
...
function buildSettingsRuntimeSummary(config: RuntimeConfig): DashboardSettingsRuntimeSummary {
  return {
...
function buildSettingsSummary(
  tenantRules: readonly TenantRules[],
  communicationTemplates: readonly DashboardSettingsCommunicationTemplate[],
  suppressionRules: readonly DashboardSettingsSuppressionRule[],
  memory: readonly MemoryEntry[],
  auditTrail: readonly DashboardMemoryAuditEntry[],
): DashboardSettingsSummary {
  return {
...
const STAGE2_PHASE_ORDER: readonly Stage2RuntimePhase[] = [
  "supervision",
  "routing",
  "planning",
  "research",
...
const STAGE2_AGENT_ORDER: readonly Stage2AgentId[] = [
  "supervisor-agent",
  "router-agent",
  "planner-agent",
  "research-agent",
...
function buildSettingsOrchestrationHistory(
  orchestrator: Stage2RuntimeOrchestrator | undefined,
  query: DashboardQuery,
): DashboardSettingsOrchestrationHistory {
...
function summarizeOrchestrationHistory(
  history: readonly Stage2InvocationRecord[],
): DashboardSettingsOrchestrationHistorySummary {
...
function createPhaseCountMap(): Record<Stage2RuntimePhase, number> {
  return STAGE2_PHASE_ORDER.reduce((accumulator, phase) => {
    accumulator[phase] = 0;
    return accumulator;
  }, {} as Record<Stage2RuntimePhase, number>);
}

function createAgentCountMap(): Record<Stage2AgentId, number> {
  return STAGE2_AGENT_ORDER.reduce((accumulator, agentId) => {
    accumulator[agentId] = 0;
    return accumulator;
  }, {} as Record<Stage2AgentId, number>);
}

function mapSettingsOrchestrationHistoryItem(record: Stage2InvocationRecord): DashboardSettingsOrchestrationHistoryItem {
  return {
...
function buildTenantRules(
  ...
  return PRODUCT_ORDER.map((product) => {
    const productTemplates = communicationTemplates
      ...
      .reduce<Record<string, string>>((accumulator, template) => {
...
function buildSettingsCommunicationTemplates(memory: readonly MemoryEntry[]): readonly DashboardSettingsCommunicationTemplate[] {
  const leadRecoveryTemplates = extractLeadRecoveryTemplates(memory).map((template) => ({
    key: `lead-recovery:${template.scenario}`,
    title: template.title,
    body: template.body,
    tone: template.tone,
    product: "lead-recovery" as const,
    editable: template.editable,
    source: template.sourceJobId ? "memory" as const : "runtime" as const,
  ...
  const genericTemplates = mapMemoryItems(memory)
    ...
    .map((entry) => {
      ...
      return {
        ...
        product,
        editable: entry.editable,
        source: entry.sourceJobId ? "memory" as const : "runtime" as const,
        updatedAt: entry.updatedAt,
      };
    });
...
function buildSettingsSuppressionRules(memory: readonly MemoryEntry[]): readonly DashboardSettingsSuppressionRule[] {
  const defaultRules: DashboardSettingsSuppressionRule[] = [
    {
...
function dedupeTemplates(templates: readonly DashboardSettingsCommunicationTemplate[]): readonly DashboardSettingsCommunicationTemplate[] {
  const seen = new Set<string>();
  const deduped: DashboardSettingsCommunicationTemplate[] = [];
...
function dedupeSuppressionRules(rules: readonly DashboardSettingsSuppressionRule[]): readonly DashboardSettingsSuppressionRule[] {
  const seen = new Set<string>();
  const deduped: DashboardSettingsSuppressionRule[] = [];
...
function normalizeSettingsTone(value: unknown): DashboardSettingsCommunicationTemplate["tone"] {
  if (value === "warm" || value === "urgent") {
    return value;
  }
  return "business-safe";
...
function extractWindowHours(entry: DashboardMemoryItem): number | undefined {
  const value = typeof entry.value === "object" && entry.value ? (entry.value as Record<string, unknown>) : undefined;
