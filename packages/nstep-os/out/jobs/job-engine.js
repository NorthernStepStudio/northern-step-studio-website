...
export function createJobEngine(deps) {
    ...
    function addScratchpad(job, phase, title, note, data, context) {
    ...
    function fingerprintAction(phase, source, title, note, metadata) {
    ...
    function updateWorldState(job, phase, source, title, note, data, context) {
        ...
        const actionHistory = existing
            ? worldState.actionHistory.map((entry) => entry.fingerprint === fingerprint
                ? {
                    ...entry,
                    count: repeatedCount,
                    lastAt: now,
        ...
        const observationModifiedPaths = Array.isArray(metadata.modifiedPaths)
            ? metadata.modifiedPaths.filter((item) => typeof item === "string")
            : [];
        const observationFailingTests = Array.isArray(metadata.failingTests)
            ? metadata.failingTests.filter((item) => typeof item === "string")
            : [];
    ...
    function shouldTriggerCircuitBreaker(job, fingerprint) {
    ...
    function recordWorldEvent(job, phase, source, title, note, data, context) {
    ...
    function createContext(job, route) {
        const role = job.goal.requestedByRole || "operator";
        const stage3Tools = tools;
        const scopedTools = typeof stage3Tools.scope === "function"
            ? stage3Tools.scope({
                jobId: job.jobId,
    ...
    function createPlanningContext(route) {
        return {
            config: deps.config,
            logger,
            route,
    ...
    function buildStepStates(planSteps) {
    ...
    async function enqueueJob(job, reason) {
        job.status = "queued";
        job.error = undefined;
        await saveJob(job);
        await deps.stores.queue.enqueue(job, reason);
    }
    async function finalizeQueueState(job) {
        try {
    ...
    async function ensureRoute(job) {
        if (job.route) {
            return job.route;
        }
    ...
    async function ensurePlan(job, route) {
    ...
    async function runWorkflow(jobId) {
        ...
        try {
            ...
            if (supervisionOutcome && supervisionOutcome.output.verdict !== "approved") {
                ...
                addScratchpad(job, "system", "Supervisor review", `${supervisionOutcome.output.verdict}: ${supervisionOutcome.output.findings.join(" ")}`, {
                ...
                if (!job.escalation) {
                    job.escalation = buildJobEscalation(job, `Supervisor review requested adjustment during ${supervisionOutcome.output.targetPhase}.`, "medium", "policy", job.goal.requestedByRole || "operator", {
            ...
            if (planSupervisionOutcome && planSupervisionOutcome.output.verdict !== "approved") {
                ...
                addScratchpad(job, "system", "Plan review", `${planSupervisionOutcome.output.verdict}: ${planSupervisionOutcome.output.findings.join(" ")}`, {
                ...
                if (!job.escalation) {
                    job.escalation = buildJobEscalation(job, `Supervisor review requested adjustment during ${planSupervisionOutcome.output.targetPhase}.`, "medium", "policy", job.goal.requestedByRole || "operator", {
            ...
            for (let index = 0; index < job.steps.length;) {
                ...
                if (step.approvalRequired && !job.approvedStepIds.includes(step.id)) {
                    ...
                    job.escalation = upsertEscalation(job, buildJobEscalation(job, `Step ${step.id} requires approval before execution.`, "medium", "approval", job.goal.requestedByRole || "operator", {
                        stepId: step.id,
                        stepType: step.type,
                    }));
                    await saveJob(job);
                ...
                const actionRestriction = evaluateActionRestriction(step, route, job.goal.requestedByRole || "operator", job.approvedStepIds.includes(step.id) || job.approvalStatus === "approved");
                if (!actionRestriction.allowed) {
                    step.status = "waiting_approval";
                    job.status = "waiting_approval";
                    job.approvalStatus = "pending";
                    job.escalation = upsertEscalation(job, buildJobEscalation(job, actionRestriction.reason, route.riskLevel, "policy", actionRestriction.requiredRole || job.goal.requestedByRole || "operator", {
                        stepId: step.id,
                        stepType: step.type,
                        tool: step.tool,
                    }));
                ...
                if (shouldTriggerCircuitBreaker(job, executionWorld.fingerprint)) {
                    ...
                    job.escalation = upsertEscalation(job, buildJobEscalation(job, job.error, "high", "retry-exhausted", job.goal.requestedByRole || "operator", {
                        stepId: step.id,
                        stepType: step.type,
                ...
                if (result.status === "blocked") {
                    ...
                    job.status = "waiting_approval";
                    job.approvalStatus = "pending";
                    job.escalation = upsertEscalation(job, buildJobEscalation(job, result.message, route.riskLevel, "policy", job.goal.requestedByRole || "operator", {
                        stepId: step.id,
                        stepType: step.type,
                        tool: step.tool,
                    }));
                ...
                step.retry = buildRetryModel(step, retryLimit);
                job.escalation = upsertEscalation(job, buildJobEscalation(job, `Retry boundary reached for step ${step.id}. ${result.message}`, "high", "retry-exhausted", job.goal.requestedByRole || "operator", {
                    stepId: step.id,
                    stepType: step.type,
                    attempts: step.attempts,
            ...
            if (verification.outcome === "human_review_required") {
                ...
                assertWorkflowResult(result);
                job.result = result;
                job.escalation = upsertEscalation(job, buildJobEscalation(job, "Verification requested human review.", "medium", "verification", job.goal.requestedByRole || "operator", {
                    outcome: verification.outcome,
            ...
            if (verification.outcome === "retry_required" || verification.outcome === "rollback_required") {
                ...
                assertWorkflowResult(result);
                job.result = result;
                job.escalation = upsertEscalation(job, buildJobEscalation(job, job.error, "high", verification.outcome === "retry_required" ? "retry-exhausted" : "verification", job.goal.requestedByRole || "operator", {
                    outcome: verification.outcome,
                    findings: verification.findings.slice(0, 10),
                }));
            ...
            assertWorkflowResult(result);
            job.result = result;
            job.escalation = upsertEscalation(job, buildJobEscalation(job, "Verification failed.", "high", "verification", job.goal.requestedByRole || "operator", {
                outcome: verification.outcome,
                findings: verification.findings.slice(0, 10),
            }));
            ...
            job.status = "failed";
            job.error = message;
            job.escalation = upsertEscalation(job, buildJobEscalation(job, `Workflow execution failed: ${message}`, "high", "verification", job.goal.requestedByRole || "operator", {
                error: message,
...
function addOrchestrationLog(job, record) {
...
function buildOrchestrationContext(job, route, step) {
    return {
        jobId: job.jobId,
        tenantId: job.tenantId,
        product: job.goal.product,
...
function upsertEscalation(job, escalation) {
...
function resolveEscalation(job, status, reason) {
    if (!job.escalation) {
        return undefined;
    }
    return {
...
//# sourceMappingURL=job-engine.js.map