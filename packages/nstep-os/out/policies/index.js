...
const EXTERNAL_TOOLS = new Set(["browser", "sms", "email", "api", "scraping"]);
export function buildApprovalPolicy(approvalThreshold, minimumRole = "operator") {
    return {
        minimumRole,
        approvalThreshold,
...
export function evaluateGoalRisk(goal) {
...
export function evaluateLeadRecoveryRisk(goal) {
    const text = `${goal.goal} ${goal.constraints.join(" ")}`.toLowerCase();
    if (/(delete|destroy|payment|refund|send money|wire transfer)/.test(text)) {
        return "critical";
    }
...
export function evaluateNexusBuildRisk(goal) {
    const text = `${goal.goal} ${goal.constraints.join(" ")}`.toLowerCase();
...
export function evaluateProvLyRisk(goal) {
    const text = `${goal.goal} ${goal.constraints.join(" ")}`.toLowerCase();
    if (/(delete|destroy|payment|refund|send money|wire transfer)/.test(text)) {
        return "critical";
    }
...
export function evaluateApprovalPolicy(goal, route, approvalThreshold = "high") {
    ...
    const requiresApproval = goal.mode === "assist" ||
...
export function evaluateActionRestriction(step, route, principalRole = "operator", approvalGranted = false) {
...
export function evaluateTenantIsolation(resourceTenantId, requestedTenantId, principalRole = "operator") {
...
export function buildJobEscalation(job, reason, severity, source, ownerRole, metadata = {}) {
...
export function requiresApproval(route, goal) {
    return evaluateApprovalPolicy(goal, route).requiresApproval;
}
export function requiresStepApproval(step, route) {
    if (step.approvalRequired) {
        return true;
    }
    return route.riskLevel === "critical" && step.tool !== "database";
}
export function evaluateLeadRecoverySafety(input) {
