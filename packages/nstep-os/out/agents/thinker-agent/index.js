...
const thinkerResponsibilities = [
    defineStage2Responsibility("Problem framing", "Breaks a goal into the actual problem shape before a plan is written.", ["goal input", "route context", "memory notes"]),
    defineStage2Responsibility("Reasoning capture", "Writes compact reasoning notes that can feed planning, memory, and operator review.", ["scratchpad", "planning handoff"]),
    defineStage2Responsibility("Risk surfacing", "Calls out ambiguity, dependencies, and assumptions before execution starts.", ["planning", "verification prep"]),
];
const thinkerPermissions = [
    defineStage2Permission("thinking", ["reason"], "May analyze goals and produce reasoning notes for downstream planning.", {
        mayUseExternalTools: false,
        requiresApprovalForExternalActions: false,
    }),
];
export function createThinkerAgent(context, bridge) {
    ...
    return {
        ...
        think(request) {
            ...
            if (prioritizedMemory.length) {
                reasoning.push(`Memory tiers loaded: ${memoryHierarchy.semantic.length} semantic, ${memoryHierarchy.procedural.length} procedural, ${memoryHierarchy.episodic.length} episodic item(s).`);
...
function collectRisks(goal, route) {
    const risks = new Set();
...
function collectAssumptions(goal, route, memoryHierarchy) {
    ...
    if (memoryHierarchy?.semantic.length || memoryHierarchy?.procedural.length || memoryHierarchy?.episodic.length) {
        assumptions.add(`Memory loaded: ${memoryHierarchy.semantic.length} semantic, ${memoryHierarchy.procedural.length} procedural, ${memoryHierarchy.episodic.length} episodic item(s).`);
...
function summarizeMemoryValue(value) {
    ...
    return Object.entries(value)
        .slice(0, 3)
        .map(([key, item]) => `${key}=${summarizeMemoryValue(item) || String(item)}`)
        .filter(Boolean)
        .join(", ");
}
