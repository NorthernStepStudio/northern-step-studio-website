...
const sourceGathererResponsibilities = [
    defineStage2Responsibility("Source collection", "Collects source candidates from the allowed retrieval channels without doing the reasoning itself.", ["research sources", "browser adapter", "api adapter"]),
    defineStage2Responsibility("Evidence normalization", "Normalizes source metadata into a compact evidence list that the research agent can reason over.", ["research handoff"]),
    defineStage2Responsibility("Retrieval boundary", "Keeps the evidence-gathering phase separate from the reasoning phase.", ["research", "tool adapter boundary"]),
];
const sourceGathererPermissions = [
    defineStage2Permission("sources", ["gather", "research"], "May gather source candidates and normalize evidence for downstream reasoning.", {
        mayUseExternalTools: true,
        requiresApprovalForExternalActions: true,
    }),
];
export function createSourceGathererAgent(context, _bridge) {
...
function buildKnowledgeQuery(request) {
...
function deriveTitle(source, index) {
