...
import { chunkKnowledgeCorpus, summarizeKnowledgeCoverage } from "../knowledge/index.js";
async function main() {
    ...
    const sources = await Promise.all(sourceFiles.map(async (sourcePath) => ({
        sourcePath: toPosixPath(path.relative(workspaceRoot, sourcePath)),
        markdown: await readFile(sourcePath, "utf8"),
    })));
    const chunks = chunkKnowledgeCorpus(sources);
    ...
    console.log(`Lane coverage: ${coverage.presentLaneDocuments}/${coverage.expectedLaneDocuments} lane docs present (${coverage.coveragePercent}%), missing ${coverage.missingLaneDocuments}.`);
