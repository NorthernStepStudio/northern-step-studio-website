...
import { createDashboardService } from "./dashboard/index.js";
...
export async function startNStepOsServer(runtime, requestedPort) {
        ...
        try {
            ...
            if (request.method === "POST" && url.pathname === "/v1/webhooks/twilio/missed-call") {
                ...
                sendJson(response, 200, {
                    status: "processed",
                    job,
                    event: goal.payload?.leadRecovery?.event,
                });
                return;
            }
...
async function readFlexibleBody(request) {
    const raw = await readRawBody(request);
...
function parseFlexibleBody(raw, contentType) {
    if (!raw) {
        return {};
    }
...
async function buildObjectFromFormData(formData) {
    const body = {};
    const visualAssets = [];
    const attachments = [];
    const receipts = [];
...
function appendFormField(body, key, value) {
...
async function buildVisualAssetFromFile(fieldName, file) {
...
function inferVisualAssetKind(fieldName, fileName, mimeType) {
...
function shouldTreatAsReceipt(fieldName, file) {
...
function buildTwilioLeadRecoveryGoal(body) {
    const goal = body.goal;
...
function buildNexusBuildGoal(body, requestedByRole = "operator") {
    const goal = body.goal;
    ...
    const constraints = Array.isArray(rawConstraints)
        ? rawConstraints.filter((item) => typeof item === "string" && item.trim().length > 0)
        : [
    ...
    const buildName = stringField(body, "buildName") ||
...
function buildProvLyGoal(body, requestedByRole = "operator") {
    const goal = body.goal;
...
function buildProvLyUploadSummary(job, goal, body) {
    ...
    const extraction = coerceObject(dashboardSummary.visualExtraction) ||
...
function buildSharedAdapterGoal(body, origin, requestedByRole = "system") {
    const goal = coerceObject(body.goal);
    if (goal) {
...
function buildTwilioLeadRecoveryEvent(body, tenantId) {
...
function originToHealthUrl(origin) {
    try {
...
function buildTwilioBrandProfile(body, tenantId) {
...
function buildTwilioLeadRecord(body, tenantId, phone) {
...
function isMissedCallEvent(body) {
...
function readDashboardQuery(searchParams) {
    return {
...
function resolveDashboardProduct(value) {
...
function resolveWorkflowKey(value) {
    if (value === "lead-recovery" || value === "nexusbuild" || value === "provly" || value === "neurormoves" || value === "shared") {
        return value;
    }
    return undefined;
}
function resolveJobStatuses(values) {
    const statuses = values.filter((value) => isJobStatus(value));
    if (statuses.length === 0) {
        return undefined;
    }
    return statuses.length === 1 ? statuses[0] : statuses;
}
function resolveApprovalStatuses(values) {
    const statuses = values.filter((value) => isApprovalStatus(value));
    if (statuses.length === 0) {
        return undefined;
    }
    return statuses.length === 1 ? statuses[0] : statuses;
}
function resolveExecutionLane(value) {
...
function readQueryValues(searchParams, key) {
    const values = searchParams.getAll(key).flatMap((value) => value.split(","));
    return values.map((value) => cleanQueryValue(value)).filter((value) => Boolean(value));
}
...
function resolveKnowledgeSearchLimit(value) {
...
function resolvePrincipalRole(request, fallback) {
...
function resolveActorSubjectId(request, body) {
...
function resolveTenantFilter(searchParams, request) {
...
function resolveNexusBuildUseCase(body) {
...
function resolveProvLyOperation(body) {
    const operation = stringField(body, "operation") || stringField(body, "workflowType") || stringField(body, "intent");
    if (operation === "inventory-intake" ||
...
function resolveProvLyExportFormat(body) {
...
function resolveProvLyReminderMode(body) {
