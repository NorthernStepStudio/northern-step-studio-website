...
export function createMemoryService(store, logger = createNoopLogger()) {
    async function persist(entries, options) {
...
export async function createJobMemory(workflow, job, context) {
    return workflow.createMemory(job, context);
}
export async function editMemoryEntry(store, memoryId, request) {
    ...
    const next = {
        ...
        category: (request.category?.trim() || existing.category),
...
export async function appendMemoryAudit(store, memoryId, audit) {
    const existing = await store.get(memoryId);
    if (!existing) {
        throw new Error(`Memory entry ${memoryId} was not found.`);
    }
...
function appendAuditTrail(existing, audit) {
    return [...(existing || []), audit];
}
function buildMemoryDiff(existing, request) {
    const diff = {};
...
export function createMemoryAuditRecord(actorRole, note, sourceJobId, sourceStepId) {
    return {
...
export function createMemoryLesson(input) {
    return {
...
export function classifyMemoryTier(category) {
...
export function buildMemoryHierarchy(memory, limit = 12) {
...
export function selectMemoryForReasoning(memory, limit = 12) {
    const ordered = [...memory].sort((left, right) => {
        const leftAt = Date.parse(left.updatedAt || left.createdAt || "");
        const rightAt = Date.parse(right.updatedAt || right.createdAt || "");
        return (Number.isFinite(rightAt) ? rightAt : 0) - (Number.isFinite(leftAt) ? leftAt : 0);
    ...
    const selected = [];
    const seen = new Set();
    const tierPriority = ["semantic", "procedural", "episodic"];
...
export function inferMemoryLesson(job, entry) {
    if (entry.lesson) {
        return entry.lesson;
    }
...
export function createMemoryEntryId(prefix = "memory") {
    return `${prefix}_${randomUUID()}`;
}
export async function persistMemoryEntries(store, entries, options = {}) {
    const now = new Date().toISOString();
    let created = 0;
    let updated = 0;
    const ids = [];
...
function buildPersistenceAudit(entry, existing, options, at) {
    return {
        at,
...
function entryToEditRequest(entry, options) {
    return {
