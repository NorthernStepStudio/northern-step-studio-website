...
export function inferUseCase(text) {
...
export function inferCategory(value, fallbackText = "") {
...
export function inferSocket(value, fallbackText = "") {
...
export function inferMemoryType(value, fallbackText = "") {
...
export function inferFormFactor(value, fallbackText = "") {
...
export function inferPsuWattage(value, fallbackText = "") {
...
export function inferMemoryCapacityGb(value, fallbackText = "") {
...
export function inferStorageCapacityGb(value, fallbackText = "") {
    return inferMemoryCapacityGb(value, fallbackText);
}
export function inferGpuLengthMm(value, fallbackText = "") {
...
export function inferCpuTdp(value, fallbackText = "") {
...
export function buildPriceText(parts) {
    return parts
        .map((part) => `${part.name} ${part.brand || ""} ${part.model || ""} ${stringifyValue(part.specs)}`)
        .join(" ");
}
export function scoreCpuPart(part) {
    if (!part) {
        return 0;
    }
...
export function scoreGpuPart(part) {
    if (!part) {
        return 0;
    }
    const text = normalizeText(`${part.name} ${part.brand || ""} ${part.model || ""}`);
...
export function scoreMemoryPart(part) {
    if (!part) {
        return 0;
    }
...
export function scoreStoragePart(part) {
    if (!part) {
        return 0;
    }
    const text = normalizeText(`${part.name} ${part.brand || ""} ${part.model || ""} ${stringifyValue(part.specs)}`);
...
export function scorePowerSupply(part) {
    if (!part) {
        return 0;
    }
...
export function estimatePowerDraw(parts) {
...
export function inferPsuMargin(wattage, estimatedDraw) {
    if (!wattage || wattage <= 0) {
        return undefined;
    }
    return Math.round(((wattage - estimatedDraw) / wattage) * 100);
}
export function getBuildPrice(parts) {
...
export function buildNormalizedBuildLabel(snapshot) {
