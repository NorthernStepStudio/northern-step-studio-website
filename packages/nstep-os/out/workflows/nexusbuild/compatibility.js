import { estimatePowerDraw, inferFormFactor, inferGpuLengthMm, inferMemoryType, inferPsuWattage, inferSocket, normalizeText, scoreCpuPart, scoreGpuPart, stringifyValue, } from "./catalog.js";
const ISSUE_PENALTIES = {
    info: 4,
    warning: 12,
    error: 25,
};
export function reviewNexusBuildCompatibility(intake) {
    ...
    const score = Math.max(0, Math.min(100, Math.round(100 - issues.reduce((total, issue) => total + ISSUE_PENALTIES[issue.severity], 0))));
...
function supportsFormFactor(caseFormFactor, boardFormFactor) {
...
function formFactorRank(formFactor) {
