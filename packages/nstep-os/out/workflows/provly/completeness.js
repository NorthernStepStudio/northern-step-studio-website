...
import { DEFAULT_PROVLY_UNASSIGNED_ROOM, normalizeText } from "./catalog.js";
export function checkProvLyCompleteness(intake, classification, normalized) {
...
function scoreItem(item, attachments, receipts, intake) {
    ...
    const severity = score < 70 ? "error" : "warning";
    return {
        itemId: item.itemId,
        label: item.name,
        score,
...
function buildRuleNotes(intake, completeness, priorityItems, items, attachments, receipts) {
    const notes = [];
...
function buildReminderLine(label, missingFields) {
    if (missingFields.length === 0) {
        return `${label} is ready.`;
    }
    return `${label} still needs ${missingFields.join(", ")}.`;
}
function scoreFieldCategory(field) {
    switch (field) {
        case "receipt":
            return "receipt";
        case "photo":
...
function defaultResolution(field) {
    switch (field) {
        case "receipt":
...
function isSerialSensitive(item) {
