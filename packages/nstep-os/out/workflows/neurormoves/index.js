...
function extractNeuroMovesInput(goal) {
    const payload = goal.payload;
    const raw = payload?.neurormoves ?? goal.payload;
    return {
        goal,
...
function planNeuroMoves(input, _context) {
    const steps = [
        {
            id: "s1",
...
async function executeNeuroMovesStep(step, context) {
...
function verifyNeuroMovesJob(job, input) {
    const findings = [];
...
function buildSupportProfile(input) {
    return {
        childName: input.childName,
        ageGroup: input.ageGroup,
        focus: input.focus,
...
function buildParentSummary(input, routine) {
    return [
...
function buildNeuroMovesMemory(input, key, jobId) {
...
export function createNeuroMovesWorkflow() {
    return {
        key: "neurormoves",
        title: "NeuroMoves",
